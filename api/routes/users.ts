...
router.get('/me', (req: Request, res: Response): void => {
  try {
    ...
    if (!userId) { res.status(400).json({ success: false, error: '缺少userId参数' }); return }
    ...
    const pinnedBadges = db.prepare('SELECT b.*, ub.pinned, ub.unlocked_at FROM user_badges ub JOIN badges b ON ub.badge_id = b.id WHERE ub.user_id = ? AND ub.pinned = 1 AND ub.is_duplicate = 0 ORDER BY ub.unlocked_at DESC').all(Number(userId))
    res.json({ success: true, data: { ...userWithoutPassword, stats: { badgeCount: badgeCount.count, serviceCount: serviceCount.count, honorCount: honorCount.count }, pinnedBadges } })
  } catch (error) { res.status(500).json({ success: false, error: '获取用户信息失败' }) }
})
...
router.put('/:id', (req: Request, res: Response): void => {
  try {
    ...
    if (updates.length === 0) { res.status(400).json({ success: false, error: '没有需要更新的字段' }); return }
    values.push(userId)
    db.prepare(`UPDATE users SET ${updates.join(', ')} WHERE id = ?`).run(...values)
    const updatedUser = db.prepare('SELECT * FROM users WHERE id = ?').get(userId) as any
    const { password: _, ...userWithoutPassword } = updatedUser
    res.json({ success: true, data: userWithoutPassword })
  } catch (error) { res.status(500).json({ success: false, error: '更新用户信息失败' }) }
})
