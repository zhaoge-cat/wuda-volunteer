...
router.get('/', (req: Request, res: Response): void => {
  try {
    ...
    if (userId) {
      // Personal honors for a specific user
      honors = db.prepare(
        'SELECT * FROM honors WHERE user_id = ? ORDER BY date DESC'
      ).all(Number(userId))
    } else if (period) {
      honors = db.prepare(
        'SELECT * FROM honors WHERE period = ? ORDER BY date DESC'
      ).all(period as string)
    } else if (category) {
      honors = db.prepare(
        'SELECT * FROM honors WHERE type = ? ORDER BY date DESC'
      ).all(category as string)
    } else {
